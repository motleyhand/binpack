...
asks: **if this specific node were drained, would its workload fit on the remaining nodes
without triggering a scale-up?** Drain only when the answer is yes.

That is a bin-packing feasibility check against absolute resource quantities, not a percentage
threshold. It is not algorithmically difficult. It is simply not what any existing tool does.
...
Kubernetes service. See [ADR-0006](adr-0006-why-not-a-karpenter-doks-provider.md).
