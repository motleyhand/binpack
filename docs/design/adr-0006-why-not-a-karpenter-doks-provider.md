# ADR-0006: Why not write a Karpenter provider instead
...
- The honest recommendation, stated in the README: **if you are on EKS or AKS, use Karpenter.**
  binpack is for everyone who cannot.
- This decision should be revisited if DigitalOcean ships first-party Karpenter support, in the
  way Microsoft did for AKS. That would remove binpack's DOKS audience while leaving every
  other constrained-autoscaler provider untouched.
