...
func TestDiagnoseReportsADivergedTemplateAsSuchNotAsAnUnreadableOne(t *testing.T) {
	...
	// controller. The controller is fine, and the answer is in this cluster —
	// so it is worth a code and a remedy of its own.
	...
	f := only(t, findings, engine.FindingTemplateDivergence)
	if f.Severity != engine.Warning {
		t.Errorf("severity = %s: it blocks binpack, not the cluster", f.Severity)
	}
...
func TestAStatefulSetHeldMidRolloutDivergesTheSameWay(t *testing.T) {
	// The reason this code is named for what it observed rather than for
	// admission. binpack reads a StatefulSet's template from the StatefulSet
	// itself — unlike a Deployment, whose pods are controlled by a ReplicaSet
	// carrying a template fixed for that revision — so a rollout the pods have
	// not reached leaves the template ahead of them. `updateStrategy.type:
	// OnDelete` holds that state until somebody deletes the pods, and a
	// `rollingUpdate.partition` holds it below the partition; both are
	// indefinite and both are deliberate.
	//
	// Same finding, no webhook anywhere, and nothing wrong. Asserted rather
	// than left to the prose because the alternative is a reader sent hunting
	// a MutatingWebhookConfiguration that does not exist — and because a later
	// change narrowing the divergence check to ReplicaSet-owned pods would
	// otherwise drop this case in silence.
	old := mother.Pod("default", "shard-0", mother.OnNode("a"),
		mother.ControlledBy("StatefulSet", "shard"),
		mother.WithNodeSelector("tier", "app"))

	s := cluster([]*corev1.Node{inPool("a"), inPool("b")}, []*corev1.Pod{old})
	// The updated template, which the held rollout has not applied to the pod.
	mother.TemplateFor(s.Templates, old)

	findings := engine.Diagnose(s, config())

	f := only(t, findings, engine.FindingTemplateDivergence)
	if !strings.Contains(f.Detail, "StatefulSet shard") {
		t.Errorf("detail does not name the controller whose template to compare: %q", f.Detail)
	}
	none(t, findings, engine.FindingNoTemplate)
}
